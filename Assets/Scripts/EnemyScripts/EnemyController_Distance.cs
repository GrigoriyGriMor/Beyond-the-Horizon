...
public class EnemyController_Distance : EnemyBase
{
    ...
    private void CallForHelp(float maxDistanceVisibleEnemy)
    {
        ...
        {
            if (hitCollider.GetComponent<EnemyController_Distance>())
            {
                if (!hitCollider.GetComponent<EnemyController_Distance>().currentTargetPlayer)
                {
                    Debug.Log(" Help me!!!");
                    hitCollider.GetComponent<EnemyController_Distance>().currentTargetPlayer = currentTargetPlayer;
                }
            }
        }
