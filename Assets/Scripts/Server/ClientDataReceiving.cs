...
public class ClientDataReceiving : MonoBehaviour
{
    ...
    [Header("Server IP")]
    private string ipAdress = "188.255.120.104";
    ...
    public void InitClientReceiving(PlayerController _player, string serverAdress, int _serverPort, UdpClient socket, ushort playerId) {
        ...
        int thCount;
        int workTh;
        ThreadPool.GetMaxThreads(out workTh, out thCount);
        if (ConsoleScript.Instance) ConsoleScript.Instance.AddConsoleText(workTh + " | " + thCount + " = Потоки");

        try {
            serverIP = new IPEndPoint(Dns.GetHostEntry(ipAdress).AddressList[0], serverPort);

            connectThread = new Thread(new ThreadStart(Recieve));
            connectThread.Start();
        }
        catch (Exception ex) {
    ...
    public void Recieve() {
            while (true) {
                try {
                    ...
                    _buffer_recv = clientReciever.Receive(ref serverIP);
    ...
    public IEnumerator NewPlayerCloneConnect(RPC_Object_Data data_1) {
        ...
        if (ConsoleScript.Instance) ConsoleScript.Instance.AddConsoleText("array players in scene = " + playerControllerManager.Count);
    }
    ...
    public IEnumerator NewWarriorCloneConnect(RPC_Object_Data data_1) {
        ...
        EnemyBase newW = new EnemyBase();
        newW.SetID(data_1.playerID);

        newW = SPC.ConnectNewWarriorClone(connectData.body_type);

        if (newW != null) warriorControllerManager.Add(newW);

        if (ConsoleScript.Instance) ConsoleScript.Instance.AddConsoleText("Новый warrior создан: " + data_1.playerID);
        if (ConsoleScript.Instance) ConsoleScript.Instance.AddConsoleText("array warrior in scene = " + warriorControllerManager.Count);
    }
    #endregion
