...
public class WeaponController : MonoBehaviour
{
    ...
    private void Shot()
    {
        ...
        for (int i = 0; i < fireObjPool.Count; i++)
        {
            ...
            {
                fireObjPool[0].SetActive(false);
                fireObjPool[0].SetActive(true);

                fireObjPool[0].transform.position = muzzlePoint.position;
                fireObjPool[0].transform.rotation = muzzlePoint.rotation;
                break;
            }
        }
