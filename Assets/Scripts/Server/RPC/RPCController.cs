...
public class RPCController : MonoBehaviour
{
    ...
    public void ServerPackegeInit(byte[] _buffer_recv) {
        ...
        try {
            for (int itemIndex = 0; itemIndex < activePlayerAtScene.Count; itemIndex++) {
                ...
                //определение порта для обратной передачи
    ...
    public void C_ServerStateMainRequest() {
        if (ConsoleScript.Instance) ConsoleScript.Instance.AddConsoleText("Запрос всех состояний");
        ...
        try {
            ...
            if (ConsoleScript.Instance) ConsoleScript.Instance.AddConsoleText(ex.ToString());
            return;
        }
    }
    ...
    public void RegisterNewObj(RPCObject obj) {
        if (ConsoleScript.Instance) ConsoleScript.Instance.AddConsoleText("Новый RPC зарегистрирован");
    ...
    private IEnumerator RegisterNewPlayer(RPCObject obj) {
        ...
        PlayerServerConnectData newPack = new PlayerServerConnectData();
        newPack.name = obj.player.player.playerName.text.ToCharArray();
