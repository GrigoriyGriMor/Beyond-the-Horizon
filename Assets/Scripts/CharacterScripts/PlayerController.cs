...
public class PlayerController : MonoBehaviour {
    ...
    private void Start() {
        if (PlayerParameters.Instance) PlayerParameters.Instance.SetPlayerController(this);
    }
    ...
    [HideInInspector] public ushort body_type = 0;
    ...
    private void Update() {
        ...
        if (UDPStatus == SupportClass.gameState.client || UDPStatus == SupportClass.gameState.clone) {
            transform.position = Vector3.LerpUnclamped(transform.position, currentPlayerPosition, 15 * Time.deltaTime);
            ...
            visual.transform.rotation = Quaternion.LerpUnclamped(visual.transform.rotation, tr, 15 * Time.deltaTime);
        }
    ...
    {
        if (aiming && mode == SupportClass.PlayerStateMode.Combat)
        {
            SetNewAnimLayerW(1);
        ...
        if (currentInteractiveItem != null) {
            switch (currentInteractiveItem.itemType) {
                ...
                case SupportClass.interactiveItemType.Car:
                    if (currentInteractiveItem.GetComponentInParent<CarBase>()) currentInteractiveItem.GetComponentInParent<CarBase>().StartCar(this);
                    break;
