...
public class ClientDataInput : MonoBehaviour
{
    ...
    [Header("Server IP")]
    [SerializeField] private string ipAdress = "188.255.120.104";
    ...
    private void OnSceneLoaded(Scene scene, LoadSceneMode mode) {
        ThreadPool.QueueUserWorkItem((q) => StartCoroutine(LogIn()));
    }
    ...
    private IEnumerator LogIn() {
        ...
        keepAlive.proto = 0xBEBE;
        keepAlive.version = 0x0000;
        keepAlive.id = (ushort)serverInfo.id;
        keepAlive.token = BitConverter.ToUInt64(Encoding.UTF8.GetBytes(serverInfo.session), 0);
        ...
        ThreadPool.QueueUserWorkItem((q) => StartCoroutine(SendData()));
        ThreadPool.QueueUserWorkItem((q) => StartCoroutine(SendAliveControl(keepAlive)));
    }
    ...
    private IEnumerator SendAliveControl(KeepAlive _keepAlive) {
        yield return new WaitForSeconds(0.5f);
        ...
        ThreadPool.QueueUserWorkItem((q) => StartCoroutine(SendAliveControl(_keepAlive)));
    }
    ...
    public void SetNewMoveAxis(Vector2 Axis)//получаем смещение axis
    {
